...
rootfs: base.tar
	...
	sudo cp apparmor.service rootfs/etc/systemd/system/apparmor.service
...
base.tar:
	...
	docker run --net=host --name soluswsl silkeh/solus:ypkg /bin/bash -c "eopkg up -y; eopkg it -y apparmor bzip2 cmake dialog dos2unix efivar elfutils elfutils-devel libelf-devel iptables iproute2 keychain lolcat openssh rsync socat sqlite3 wget xdg-utils; eopkg dc; mkdir -p /usr/local/bin; git clone https://github.com/cmatsuoka/figlet.git; cd figlet; make && make install; cd && rm -rf /figlet; git clone https://github.com/sileshn/dwarves.git; cd dwarves; mkdir build && cd build; cmake -DCMAKE_INSTALL_PREFIX=/usr -D__LIB=lib ..; make install; cd && rm -rf dwarves; touch /usr/share/defaults/etc/profile.d/custom.sh; touch /etc/environment; rm /usr/share/defaults/etc/profile.d/10-path.sh; mkdir -p /usr/lib/wsl; systemctl mask systemd-firstboot"
